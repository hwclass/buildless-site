...
class Router extends Resolver {
      ...
      Object.assign(
        {
          ...
          baseUrl:
            baseHref &&
            Resolver.__createUrl(baseHref, document.URL).pathname.replace(
              /[^\/]*$/,
              ""
            )
        },
      ...
      .catch(error => {
        if (renderId === this.__lastStartedRenderId) {
          ...
          this.location = createLocation(
            Object.assign(context, { resolver: this })
          );
          fireRouterEvent(
            "error",
            Object.assign({ router: this, error }, context)
          );
          throw error;
        }
      });
  ...
  __runOnBeforeCallbacks(newContext) {
    ...
    if (previousChain.length) {
      ...
      if (newContext.__skipAttach) {
        ...
        for (let i = newChain.length - 1; i >= 0; i--) {
          if (
            previousChain[i].path !== newChain[i].path ||
            newContext.search !== previousContext.search
          ) {
        ...
        for (let i = 0; i < newChain.length; i++) {
          ...
          previousChain[i].element.location = createLocation(
            newContext,
    ...
    for (
      let i = newContext.__divergedChainIndex;
      !newContext.__skipAttach && i < newChain.length;
      i++
    ) {
      callbacks = this.__runOnBeforeEnterCallbacks(
        callbacks,
    ...
    return callbacks
      .then(result => {
        if (this.__isLatestRender(newContext)) {
          const afterLeaveFunction = amend(
            "onBeforeLeave",
  ...
  __isReusableElement(element, otherElement) {
    if (element && otherElement) {
      return this.__createdByRouter.get(element) &&
        this.__createdByRouter.get(otherElement)
  ...
  __addAppearingContent(context, previousContext) {
    ...
    const deepestCommonParent = this.__copyUnchangedElements(
      context,
      previousContext
    );
...
const FlowClients =
  window.Vaadin && window.Vaadin.Flow && window.Vaadin.Flow.clients;
